    ...
    struct http_response *rsp, enum http_final_call final_data, void *user_data)
{
    ...
    if (final_data == HTTP_DATA_MORE) {
        LOG_INF("Partial data received (%zd bytes)", rsp->data_len); // NOLINT
        LOG_ERR("HTTP reply is too long for rx buffer."); // NOLINT
        *request_ok = false;
    } else if (final_data == HTTP_DATA_FINAL) {
        LOG_INF("All the data received (%zd bytes)", rsp->data_len); // NOLINT
    ...
    LOG_INF("Response status %s", rsp->http_status); // NOLINT
}

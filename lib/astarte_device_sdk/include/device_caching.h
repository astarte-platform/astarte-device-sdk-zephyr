 ...
 */
astarte_result_t astarte_device_caching_store_introspection(const char *intr, size_t intr_size);
 ...
 * @return ASTARTE_RESULT_OK if successful, otherwise an error code.
 */
astarte_result_t astarte_device_caching_check_introspection(const char *intr, size_t intr_size);
 ...
 * @return ASTARTE_RESULT_OK if successful, otherwise an error code.
 */
astarte_result_t astarte_device_caching_store_property(
 ...
 * #astarte_device_caching_destroy_loaded_property after its usage has ended.
 *
 * @param[in] interface_name Interface name
 * @param[in] path Property path
 ...
 * @return ASTARTE_RESULT_OK if successful, otherwise an error code.
 */
astarte_result_t astarte_device_caching_load_property(const char *interface_name, const char *path,
    uint32_t *out_major, astarte_individual_t *individual);

/**
 * @brief Destroy individual data for a previously loaded property.
 *
 * @details Use this function to free the memory allocated by #astarte_device_caching_load_property.
 *
 * @param[out] individual Astarte individual loaded by #astarte_device_caching_load_property.
 */
void astarte_device_caching_destroy_loaded_property(astarte_individual_t individual);
 ...
 * @return ASTARTE_RESULT_OK if successful, otherwise an error code.
 */
astarte_result_t astarte_device_caching_delete_property(
 ...
 * @return ASTARTE_RESULT_OK if successful, otherwise an error code.
 */
astarte_result_t astarte_device_caching_get_properties_string(char *output, size_t *output_size);

...
#define AUTH_HEADER_CRED_SECRED_SIZE 69 // Fixed string (24) + cred secret (44) + NULL (1)
    ...
    int32_t timeout_ms, const char *cred_secr, char *out_url, size_t out_url_size)
{
    ...
    char auth_header[AUTH_HEADER_CRED_SECRED_SIZE] = { 0 };
    strncpy(auth_header, "Authorization: Bearer ", strlen("Authorization: Bearer ") + 1);
    strncat(
        auth_header, cred_secr, AUTH_HEADER_CRED_SECRED_SIZE - strlen("Authorization: Bearer "));
    strncat(auth_header, "\r\n",
        AUTH_HEADER_CRED_SECRED_SIZE - strlen("Authorization: Bearer ") - strlen(cred_secr));
    ...
    unsigned char *privkey_buf, size_t privkey_buf_size, char *pem_crt, size_t pem_crt_size)
{
    ...
    if (privkey_buf_size < ASTARTE_CRYPTO_PRIVKEY_BUFFER_SIZE) {
        LOG_ERR("Insufficient output buffer size for client private key."); // NOLINT
        return ASTARTE_ERR_INVALID_PARAM;
    }
    if (pem_crt_size <= ASTARTE_PAIRING_MAX_CLIENT_CRT_LEN) {
        LOG_ERR("Insufficient output buffer size for client certificate."); // NOLINT
        return ASTARTE_ERR_INVALID_PARAM;
    }

    // Step 2: create a private key and a CSR
    astarte_err_t crypto_rc = astarte_crypto_create_key(privkey_buf, privkey_buf_size);
    ...
    crypto_rc = astarte_crypto_create_csr(privkey_buf, csr_buf, sizeof(csr_buf));
    if (crypto_rc != ASTARTE_OK) {
    ...
    return parse_get_client_certificate_response(resp_buf, pem_crt);
}
...
static astarte_err_t parse_register_device_response(char *resp_buf, char *out_cred_secr)
{
    ...
    LOG_INF("Received credentials secret: %s", parsed_json.data.credentials_secret); // NOLINT
...
static astarte_err_t parse_get_borker_url_response(char *resp_buf, char *out_url)
{
    ...
    // Copy the received credential secret in the output buffer
    // NOLINTNEXTLINE
    LOG_INF("Received broker URL: %s", parsed_json.data.protocols.astarte_mqtt_v1.broker_url);
...
static astarte_err_t parse_get_client_certificate_response(char *resp_buf, char *out_deb)
{
    ...
    // Copy the received credential secret in the output buffer
    LOG_INF("Received client certificate: %s", parsed_json.data.client_crt); // NOLINT
